...
import { ArrowRight, HandHeart, HeartHandshake, Loader2 } from "lucide-react";
import { z } from "zod";
...
function DonationForm({ onDone }: { onDone: () => void }) {
  ...
  const { submitting, run } = useSubmit(onDone);
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setErrors({});
    void run(fr ? "Merci ! Nous vous écrivons très vite." : "Thank you! We will email you shortly.");
  };

  return (
    <form onSubmit={onSubmit} noValidate className="space-y-4">
      <div className="grid gap-2">
        <Label htmlFor="d-amount">{fr ? "Montant (USD)" : "Amount (USD)"}</Label>
      ...
      <div className="grid gap-2">
        ...
        <div className="flex gap-2">
          {(["one-off", "monthly"] as const).map((f) => (
            ...
            >
              {f === "one-off" ? (fr ? "Unique" : "One-off") : fr ? "Mensuel" : "Monthly"}
            </button>
          ))}
        </div>
      </div>
      ...
      >
        ...
        {fr ? "Envoyer le don" : "Submit donation"}
      </Button>
    </form>
  );
}
...
function VolunteerForm({ onDone }: { onDone: () => void }) {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setErrors({});
    void run(
      fr ? "Merci ! Les admissions vous contacteront." : "Thank you! Admissions will contact you.",
    );
  };
