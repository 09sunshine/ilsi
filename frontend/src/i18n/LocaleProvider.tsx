...
/** Picks the right language variant from a bilingual content field safely without throwing. */
export function useLocalized() {
  const { locale } = useI18n();
  return useCallback(
    (value?: { en?: string; fr?: string } | string | null) => {
      if (!value) return "";
      if (typeof value === "string") return value;
      if (typeof value !== "object") return String(value);
      return (locale === "fr" ? value.fr || value.en : value.en || value.fr) || "";
    },
    [locale],
  );
}
