...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "twitter:title", content: "ILSI — Cohort-based training that people finish" },
      {
  ...
  useEffect(() => {
    async function loadPrograms() {
      try {
        ...
        console.warn("Could not fetch dynamic programs for home:", err);
      }
    }
    loadPrograms();
  }, []);
  ...
  return (
    <PublicShell>
      ...
      <section className="hero-dark hero-grid-lines relative isolate text-hero-foreground">
        <div className="relative mx-auto max-w-6xl px-4 pb-16 pt-24 sm:px-6 sm:pt-28 lg:pb-24 lg:pt-32">
            ...
            <div>
              ...
              <div className="mt-10">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-x-8 gap-y-3 text-hero-foreground/80">
                  ...
                  <span className="inline-flex items-center gap-2 text-sm font-semibold">
                    <BarChart3 className="size-4 text-brand-orange" aria-hidden />
                    87% {t("home.statsCompletion")}
                  </span>
                  <span className="inline-flex items-center gap-2 text-sm font-semibold">
                    <CalendarClock className="size-4 text-brand-orange" aria-hidden />
                    {t("home.heroPath")}
                  </span>
                </div>
              </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-24">
        ...
        {programList.length === 0 ? (
          <div className="mt-10 panel p-12 text-center">
            <GraduationCap className="size-10 mx-auto text-muted-foreground/60 mb-3" />
            <h3 className="text-base font-semibold">{t("programs.noMatch")}</h3>
            <p className="mt-1 text-sm text-muted-foreground">{t("programs.noMatchBody")}</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="aspect-[3/2] overflow-hidden">
                    <img
                      src={resolveMediaUrl(p.thumbnailUrl || p.thumbnail_url) || programImage[p.slug] || youngLeadersImg}
                      alt={L(p.title)}
                      loading="lazy"
                  ...
                  <div className="flex flex-1 flex-col px-5 pb-5 pt-4">
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      ...
                      <span className="ml-1 font-medium">
                        {(p.moduleCount || 6) * 5}x {t("programs.lessons")}
                      </span>
                    </div>
                    ...
                    <div className="mt-4 flex items-center justify-between border-t border-border pt-4">
                      ...
                      <span className="flex items-center gap-4 text-xs font-medium text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1.5">
                          ...
                          {p.moduleCount || 6} {t("programs.modules")}
                        </span>
                      </span>
                    </div>
                  </div>
