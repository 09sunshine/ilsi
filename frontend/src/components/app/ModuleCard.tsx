...
export function ModuleCard({
  ...
  // Use authoritative backend state if supplied, otherwise fallback to local evaluation
  const state =
    serverState ||
    (progress && now ? moduleState(modules, module, progress, now) : "LOCKED");

  const access =
    progress && now
      ? canAccessModule(modules, module.id, progress, now)
      : { allowed: state !== "LOCKED" && state !== "UPCOMING", blockingModuleOrder: module.order > 1 ? module.order - 1 : undefined, requiredScore: module.passingScore };

  const fallbackCalc =
    progress
      ? moduleLessonCompletion(module, progress)
      : { done: 0, total: module.lessons?.length || 0, percent: 0 };

  const done = doneLessons ?? fallbackCalc.done;
  const total = totalLessons ?? (module.lessons?.length || fallbackCalc.total || 1);
  const percent = completionPercent ?? (total > 0 ? Math.round((done / total) * 100) : 0);
